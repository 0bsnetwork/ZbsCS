...
namespace ZbsCS
{
    public class ExchangeTransaction : Transaction
    {
        ...
        public ExchangeTransaction(DictionaryObject tx) : base(tx)
        {
            ...
            BuyMatcherFee = Assets.ZBS.LongToAmount(tx.GetLong("buyMatcherFee"));
            SellMatcherFee = Assets.ZBS.LongToAmount(tx.GetLong("sellMatcherFee"));

            AmountAsset = node.GetAsset((tx.GetValue("order1.assetPair.amountAsset") ?? Assets.ZBS.Id).ToString());
            PriceAsset = node.GetAsset((tx.GetValue("order1.assetPair.priceAsset") ?? Assets.ZBS.Id).ToString());
        ...
        public override byte[] GetBody()
        {
            ...
            using (var writer = new BinaryWriter(stream))
            {
                ...
                writer.WriteLong(Assets.ZBS.AmountToLong(BuyMatcherFee));
                writer.WriteLong(Assets.ZBS.AmountToLong(SellMatcherFee));
        ...
        public override DictionaryObject GetJson()
        {
            var result = new DictionaryObject
            {
                ...
                {"buyMatcherFee", Assets.ZBS.AmountToLong(BuyMatcherFee)},
                {"sellMatcherFee", Assets.ZBS.AmountToLong(SellMatcherFee)}
            };
