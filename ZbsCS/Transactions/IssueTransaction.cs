...
namespace ZbsCS
{
    public class IssueTransaction : Transaction
    {
        ...
        public IssueTransaction(byte[] senderPublicKey,
            string name, string description, decimal quantity, byte decimals, bool reissuable, char chainId, decimal fee = 500m, byte[] script = null, bool scripted = false) : base(chainId, senderPublicKey)
        {
            Name = name ?? "";
        ...
        public IssueTransaction(DictionaryObject tx): base(tx)
        {
            ...
            Quantity = Assets.ZBS.LongToAmount(tx.GetLong("quantity"));
